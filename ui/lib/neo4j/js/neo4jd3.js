...
function Neo4jD3(_selector, _options) {
    ...
    function appendGraph(container) {
		zoomHandler = d3.zoom().on('zoom', function() {
	...
	function setScale(scale, x, y) {
		svg.call(zoomHandler.transform, d3.zoomIdentity.translate(x, y).scale(scale))
	}
	...
	function itemColor(d) {
		var ret = class2color(d.labels[0]);
		
		if (d.properties && d.properties.overrideColor) ret = d.properties.overrideColor;
		return ret;
	} 
    ...
    function appendOutlineToNode(node) {
        return node.append('circle')
                   .attr('class', 'outline')
                   .attr('r', function(d) {
						return d.properties.nodeRadius ? d.properties.nodeRadius : options.nodeRadius;
					})
                   .style('fill', function(d) {
                       return options.nodeOutlineFillColor ? options.nodeOutlineFillColor : itemColor(d);
                   })
                   .style('stroke', function(d) {
                       return options.nodeOutlineFillColor ? class2darkenColor(options.nodeOutlineFillColor) : '#000000'; //itemColor(d, true);
                   })
    ...
    function appendRingToNode(node) {
        return node.append('circle')
                   .attr('class', 'ring')
                   .attr('r', function(d) {
						return (d.properties.nodeRadius ? d.properties.nodeRadius : options.nodeRadius) * 1.16;
					})
                   /*.append('title').text(function(d) {
                       return toString(d);
                   })*/;
    ...
    function appendTextToNode(node) {
		return node.append('text')
                   ...
                   .attr('font-size', function(d) {
                       return icon(d) ? (nodeRad + 'px') : '10px';
                   })
                   .attr('pointer-events', 'none')
                   .attr('text-anchor', 'middle')
                   .attr('y', function(d) {
                       	var nodeRad = (d.properties.nodeRadius ? d.properties.nodeRadius : options.nodeRadius);
						return icon(d) ? (parseInt(Math.round(nodeRad * 0.32)) + 'px') : '4px';
                   })
                   .html(function(d) {
                       var _icon = icon(d);
    ...
    function appendOutlineToRelationship(r) {
        return r.append('path')
                .attr('class', 'outline')
                .attr('fill', options.lineColor) // '#a5abb6')   // Line color
                .attr('stroke', 'none');
    }
...
//                           .force('y', d3.force().strength(0.002))
                           .force('collide', d3.forceCollide().radius(function(d) {
							   var factor = d.properties.nodeRadius ? (d.properties.nodeRadius / options.nodeRadius) : 1;
                               return options.minCollision * factor;
                           }).iterations(2))
        ...
        if (node) {
			/*node.attr('fill', function(d) {
				if (currentHoveredNode) {
					if ((currentHoveredNode.id == d.source.id) ||
					    (currentHoveredNode.id == d.target.id)) {
						return options.highlightLineColor;
					}
				}
				return itemColor(d);
			});*/
        ...
        relationship.each(function(relationship) {
            ...
            outline.attr('d', function(d) {
                ...
                var center = { x: 0, y: 0 },
                    ...
                    textPadding = d.type ? 5 : 0,
                    u = unitaryVector(d.source, d.target),
                    textMargin = { x: (d.target.x - d.source.x - (textBoundingBox.width + textPadding) * u.x) * 0.5, y: (d.target.y - d.source.y - (textBoundingBox.width + textPadding) * u.y) * 0.5 },
                    n = unitaryNormalVector(d.source, d.target),
                    rotatedPointA1 = rotatePoint(center, { x: 0 + (nodeRad + 1) * u.x - n.x, y: 0 + (nodeRad + 1) * u.y - n.y }, angle),
                    rotatedPointB1 = rotatePoint(center, { x: textMargin.x - n.x, y: textMargin.y - n.y }, angle),
                    rotatedPointC1 = rotatePoint(center, { x: textMargin.x, y: textMargin.y }, angle),
                    rotatedPointD1 = rotatePoint(center, { x: 0 + (nodeRad + 1) * u.x, y: 0 + (nodeRad + 1) * u.y }, angle),
                    rotatedPointA2 = rotatePoint(center, { x: d.target.x - d.source.x - textMargin.x - n.x, y: d.target.y - d.source.y - textMargin.y - n.y }, angle),
                    rotatedPointB2 = rotatePoint(center, { x: d.target.x - d.source.x - (nodeRad + 1) * u.x - n.x - u.x * options.arrowSize, y: d.target.y - d.source.y - (nodeRad + 1) * u.y - n.y - u.y * options.arrowSize }, angle),
                    rotatedPointC2 = rotatePoint(center, { x: d.target.x - d.source.x - (nodeRad + 1) * u.x - n.x + (n.x - u.x) * options.arrowSize, y: d.target.y - d.source.y - (nodeRad + 1) * u.y - n.y + (n.y - u.y) * options.arrowSize }, angle),
                    rotatedPointD2 = rotatePoint(center, { x: d.target.x - d.source.x - (nodeRad + 1) * u.x, y: d.target.y - d.source.y - (nodeRad + 1) * u.y }, angle),
                    rotatedPointE2 = rotatePoint(center, { x: d.target.x - d.source.x - (nodeRad + 1) * u.x + (- n.x - u.x) * options.arrowSize, y: d.target.y - d.source.y - (nodeRad + 1) * u.y + (- n.y - u.y) * options.arrowSize }, angle),
