...
class Generate {
	...
	load() {
		...
		// Parent selector
		var targetSelector = n.getMoveTargetSelector();
	 ...
	 */
	generate() {
		var data = this.createData(this.options);
		...
		if (!confirm('Do you really want to create ' + data.length + ' random doduments under ' + (this.options.parentId ? this.options.parentId : 'notebook root') + '?')) {
			Notes.getInstance().showAlert('Action cancelled.', 'I', 'generateDocuments');
			return;
		}

		Notes.getInstance().showAlert('Started generating documents, please wait...', 'I', 'generateDocuments'); 
		...
		.then(function() {
			Notes.getInstance().showAlert('Successfully generated ' + data.length + ' documents', 'S', 'generateDocuments'); 
		})
		.catch(function(err) {
			if (err && err.message) Notes.getInstance().showAlert('Error generating documents: ' + err.message, 'E', 'generateDocuments');
			else Notes.getInstance().showAlert('Error generating documents', 'E', 'generateDocuments');
		});
	}
