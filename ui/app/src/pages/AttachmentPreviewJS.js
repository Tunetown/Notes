...
class AttachmentPreviewJS {
	...
	load(doc, url, blob) {
		...
		if (doc.content_type && (doc.content_type == 'application/pdf')) {
			...
			this.pdfProxy.getEngine().getDocument(blob, url)
			.then(function(pdf) {
				console.log('PDF Proxy: Loaded document in ' + (Date.now() - startTime) + 'ms, ' + Tools.convertFilesize(blob.size));
				...
				async function renderPage(index) {
					...
					.then(function(page) {
						var canvasJ = $('<canvas class="attachmentPdfiumCanvas"/>');
						...
						return page.render(canvas, w, w * ratio)
						.then(function() {
							console.log('PDF Proxy: Rendered page ' + index + ' in ' + (Date.now() - pageStartTime) + 'ms'); //, Page image size: ' + Tools.convertFilesize(data.renderSize));
				...
				renderPage(0)
				.then(function() {
					console.log('PDF Proxy: Rendered all pages in ' + (Date.now() - startTime) + 'ms');
	...
	callOptions(event) {
		...
		var n = Notes.getInstance();
		var that = AttachmentPreviewJS.getInstance();
