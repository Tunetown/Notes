/**
 * Reference driven Behaviour Handler for NoteTree
 * 
...
class ReferenceBehaviour {
	...
	beforeInit() {
		...
		$('#' + this.grid.treeRootTopSwitchContainer).append(
			// Last changed
			$('<div data-toggle="tooltip" title="Sort by last changed" class="fa fa-sort-numeric-down treeRefTopSwitchbutton" id="sortButtonLastChanged"></div>')
			...
			// Name
			$('<div data-toggle="tooltip" title="Sort by name" class="fa fa-sort-alpha-down treeRefTopSwitchbutton" id="sortButtonName"></div>')
			.on('click', function(event) {
				event.stopPropagation();
			...
			// Size
			$('<div data-toggle="tooltip" title="Sort by size" class="fa fa-sort-amount-down treeRefTopSwitchbutton" id="sortButtonSize"></div>')
			.on('click', function(event) {
				event.stopPropagation();
			...
			$('<div data-toggle="tooltip" title="Select multiple" class="fas fa-check-square treeRefTopSwitchbutton" id="multiSelectButton"></div>')
			.on('click', function(event) {
				event.stopPropagation();
	...
	getSelectorInputClass() {
		return 'ref-behaviour-selector-input';
	}
		...
		itemContent.append(
			$('<div class="' + this.getItemInnerContainerClass() + '">').append([
				...
				$('<div class="' + this.getItemHeadlineClass() + '">').append([
					...
					// Text
					$('<div class="' + this.getItemTextClass() + '"><div class="treeitemtext-ref-text">' + additionalTextBefore + doc.name + additionalTextAfter + '</div></div>').append(
						// Labels
						Document.getLabelElements(doc, 'doc-label-ref')
					),
				]),
	...
	setItemLabelAlignment(itemContent, alignRight) {
		var textElement = itemContent.find('.treeitemtext-ref-text');
	...
	getItemPreviewContentClass() {
		return "treeitempreview-content-ref";
	}
	...
	getItemMetaContentClass() {
		return "treeitemmeta-content-ref";
	}
